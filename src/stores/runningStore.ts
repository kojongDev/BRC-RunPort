...
import { LiveStats, RunStatus } from "../types/run";
...
const AUTO_PAUSE_DURATION = 8000; // 3초
...
const GPS_SIGNAL_CHECK_INTERVAL = 2000; // 2초마다 체크
...
export const useRunningStore = create<RunningState>((set, get) => ({
	...
	start: async () => {
		...
		await BackgroundGeoService.configure((location) => {
			...
			const state = get();
			// running 상태이거나, GPS 손실로 인한 일시정지 상태일 때만 처리
			if (
				state.status === "running" ||
				(state.status === "paused" && state.pauseReason === "gps_loss")
			) {
		...
		gpsSignalCheckInterval = setInterval(() => {
			...
			if (state.status === "running" && state.lastGPSUpdateTime) {
				...
				if (
					!state.gpsSignalLost &&
					timeSinceLastUpdate > GPS_LOSS_DETECTION_DELAY
				) {
					console.warn(
						"[GPS] 신호 손실 감지:",
						(timeSinceLastUpdate / 1000).toFixed(0),
						"초"
					);
					set({
						gpsSignalLost: true,
				...
				if (state.gpsSignalLost && state.gpsLossStartTime) {
					...
					if (
						lossDuration > GPS_LOSS_DETECTION_DELAY &&
	...
	addGPSPoint: (point: GPSCoordinate) => {
		...
		if (state.status === "running" && !state.isAutoPaused) {
			if (speed < AUTO_PAUSE_THRESHOLD) {
				...
				if (lowSpeedStartTime === null) {
					...
					console.log(
						"[Auto-Pause] 저속 감지 시작 - 타이머 시작. 현재 속도:",
						speed.toFixed(3),
						"m/s"
					);
				} else {
					...
					console.log(
						...
						"초 /",
						AUTO_PAUSE_DURATION / 1000,
						"초 필요"
					);

					if (elapsedTime >= AUTO_PAUSE_DURATION) {
						// 3초 이상 저속 → 자동 일시정지
						console.log(
							"⏸️ [Auto-Pause] 속도가 낮아 자동 일시정지!",
							"속도:",
							speed.toFixed(2),
							"m/s",
		...
		if (state.isAutoPaused) {
			console.log(
				"[Auto-Resume] 자동 일시정지 중 - 현재 속도:",
				speed.toFixed(3),
				"m/s",
				"/ 임계값:",
				AUTO_RESUME_THRESHOLD.toFixed(3),
				"m/s",
				`(${(AUTO_RESUME_THRESHOLD * 3.6).toFixed(1)} km/h)`
			);
			...
			if (speed >= AUTO_RESUME_THRESHOLD) {
				...
				if (highSpeedStartTime === null) {
					...
					console.log("[Auto-Resume] 고속 감지 시작 - 타이머 시작");
				} else {
					const elapsedTime = now - highSpeedStartTime;
					console.log(
						"[Auto-Resume] 고속 지속 중:",
						(elapsedTime / 1000).toFixed(1),
						"초 /",
						AUTO_RESUME_DURATION / 1000,
						"초 필요"
					);

					if (elapsedTime >= AUTO_RESUME_DURATION) {
						// 2초 이상 고속 → 자동 재개
						console.log(
							"✅ [Auto-Resume] 속도가 올라가 자동 재개!",
							"속도:",
							speed.toFixed(2),
							"m/s",
							`(${(speed * 3.6).toFixed(1)} km/h)`
				...
				// 속도가 내려가면 타이머 리셋
				if (highSpeedStartTime !== null) {
					console.log("[Auto-Resume] 속도 저하 - 타이머 리셋");
					highSpeedStartTime = null;
				}
			}
		...
		// GPS 손실로 인한 일시정지 상태에서는 GPS 포인트를 추가하지 않음
		// (자동 재개만 처리하고 경로에는 추가 안함)
		if (state.status === "paused" && state.pauseReason === "gps_loss") {
			console.log("[GPS] 일시정지 상태 - GPS 포인트는 경로에 추가하지 않음");
			return;
		}
		...
		// 현재 페이스 계산 (최근 5개 포인트)
		const currentPace =
			newRoute.length >= 2 ? CalcService.calculateCurrentPace(newRoute, 5) : 0;
